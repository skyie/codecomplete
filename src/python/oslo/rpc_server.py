...
class TestEndpoint(object):
    def test(self, ctx, arg):
        print("------ TestEndpoint.test---,arg %s,dir(ctx):%s" % (arg, dir(ctx)))
...
class RPCServer(object):
    def __init__(self):
        ...
        self.rpc_server = self.get_rpcserver()
        self.idle_thread = IdleThread()
    ...
    def get_rpcserver(self):
        ...
        server = oslo_messaging.get_rpc_server(transport, target, self.endpoints,
                                               executor='blocking')
        return server
